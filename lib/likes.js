...
import { read, write, transaction, initFirebase, parseBody, setCORS, validateInput } from '../lib/utils.js';
...
const logger = {
  info: function(message, context) { console.log(`ℹ️ [LikesService] ${message}`, context || ''); },
  warn: function(message, context) { console.warn(`⚠️ [LikesService] ${message}`, context || ''); },
  error: function(message, error, context) { console.error(`❌ [LikesService] ${message}`, context || '', error ? error.stack : ''); },
  success: function(message, context) { console.log(`✅ [LikesService] ${message}`, context || ''); },
};
...
// 添加文章点赞
export async function addArticleLike(userId, postId) {
  if (!validateInput(userId, 'id') || !validateInput(postId, 'id')) throw new ValidationError('userId/postId无效');
  ...
  try {
    ...
    if (!result.committed) {
      logger.warn(`已点赞 (userId: ${userId}, postId: ${postId})`);
...
// 移除文章点赞
export async function removeArticleLike(userId, postId) {
  if (!validateInput(userId, 'id') || !validateInput(postId, 'id')) throw new ValidationError('userId/postId无效');
  ...
  try {
    ...
    if (!result.committed) {
      logger.warn(`未点赞 (userId: ${userId}, postId: ${postId})`);
      const count = await read(likesCountPath) || 0;
...
// 检查文章点赞状态
export async function hasUserLikedArticle(userId, postId) {
  if (!validateInput(userId, 'id') || !validateInput(postId, 'id')) throw new ValidationError('userId/postId无效');
  try {
    const record = await read(`articleLikes/${userId}_${postId}`);
    return !!record;
  } catch (error) {
...
// 计算评论总赞数 (添加重试)
async function computeCommentTreeTotalLikes(postId, commentId, depth, maxRetries) {
  depth = depth || 0;
...
// 更新祖先
async function updateCommentAncestorsTotalLikes(postId, initialCommentId, depth) {
  depth = depth || 0;
  const MAX_DEPTH = 20;
...
// 添加评论点赞
export async function addCommentLike(userId, postId, commentId) {
  if (!validateInput(userId, 'id') || !validateInput(postId, 'id') || !validateInput(commentId, 'id')) throw new ValidationError('参数无效');
...
// 移除评论点赞
export async function removeCommentLike(userId, postId, commentId) {
  if (!validateInput(userId, 'id') || !validateInput(postId, 'id') || !validateInput(commentId, 'id')) throw new ValidationError('参数无效');
...
// 检查用户是否已点赞某评论
export async function hasUserLikedComment(userId, postId, commentId) {
  // 1. 输入验证：确保非空且格式正确
  if (!userId || typeof userId !== 'string' || !validateInput(userId, 'id')) {
    throw new ValidationError('userId 无效或缺失');
  }
  ...
  const commentLikePath = `commentLikes/${postId}/${commentId}/${userId}`;
  const MAX_RETRIES = 3;
  ...
  try {
    ...
    while (attempt < MAX_RETRIES) {
      try {
        ...
        logger.info(`检查点赞状态 (userId: ${userId}, postId: ${postId}, commentId: ${commentId})`, { hasLiked: !!record });
        return !!record; // 明确布尔转换
      } catch (error) {
        attempt++;
        if (error.code === 'PERMISSION_DENIED') {
          logger.error(`无权限读取点赞记录 (path: ${commentLikePath})`, error, { userId, postId, commentId });
          throw new ValidationError('无权限访问点赞记录');
        }
        logger.warn(`读取失败，尝试 ${attempt}/${MAX_RETRIES}`, error, { userId, postId, commentId });
    ...
    logger.error(`检查点赞状态失败`, error, { userId, postId, commentId });
...
// API handler
export default async function likeApiHandler(req, res) {
  setCORS(res, req);
  ...
  const { type, action, userId, postId, commentId } = body;

  if (!type || !action || !validateInput(userId, 'id') || !validateInput(postId, 'id')) {
