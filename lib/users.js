 ...
 */
const logger = {
  info: (message: string, context?: object) => console.log(`ℹ️ [UserService] ${message}`, context || ''),
  warn: (message: string, context?: object) => console.warn(`⚠️ [UserService] ${message}`, context || ''),
  error: (message: string, error?: Error, context?: object) => console.error(`❌ [UserService] ${message}`, context, error ? error.stack : ''),
  success: (message: string, context?: object) => console.log(`✅ [UserService] ${message}`, context || ''),
};
 ...
 * @returns {Promise<{ username: string; email: string; createdAt: string; role: string; uid: string; }>} - 注册成功的用户数据 (不包含密码)
 * @throws {ValidationError} 如果输入验证失败或用户名已存在
 * @throws {Error} 如果数据库操作失败
 */
export async function registerUser(userData: { username: string; email: string; password: string }): Promise<{ username: string; email: string; createdAt: string; role: string; uid: string; }> {
  const { username, email, password } = userData;

  // 1. 输入验证 (整合 validateInput 逻辑)
  if (!username || !email || !password) {
    throw new ValidationError('用户名、邮箱和密码都不能为空');
  }
  if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
    throw new ValidationError('邮箱格式不正确');
  }
  // 密码强度要求：至少8位，包含大小写字母、数字 (新的 `user.js` 示例并未提供具体的密码强度，沿用之前的更强约束)
  if (password.length < 8 || !/[a-z]/.test(password) || !/[A-Z]/.test(password) || !/\d/.test(password)) {
    throw new ValidationError('密码太弱：至少8位，包含大小写字母和数字');
  }

  const userPath = `users/${username}`; // 使用 username 作为唯一ID，或者可以考虑 Firebase Auth 的 UID
 ...
 * @returns {Promise<{ username: string; email: string; createdAt: string; role: string; lastLoginAt: string | null; uid: string; }>} - 登录成功的用户数据 (不包含密码)
 * @throws {ValidationError} 如果输入无效、用户不存在或密码错误
 * @throws {Error} 如果数据库操作失败
 */
export async function loginUser(credentials: { username: string; password: string }): Promise<{ username: string; email: string; createdAt: string; role: string; lastLoginAt: string | null; uid: string; }> {
  const { username, password } = credentials;

  // 1. 输入验证 (整合 validateInput 逻辑)
  if (!username || !password) {
    throw new ValidationError('用户名和密码都不能为空');
  }
  ...
  try {
    ...
    return { ...userWithoutPassword, uid: username} as { username: string; email: string; createdAt: string; role: string; lastLoginAt: string | null; uid: string; }; // 假设 username 就是 uid，并添加 uid
  } catch (error: unknown) {
    if (error instanceof ValidationError) {
      throw error;
    }
 ...
 * @returns {Promise<{ username: string; email: string; createdAt: string; role: string; lastLoginAt: string | null; uid: string; } | null>}
 * @throws {ValidationError} 如果输入无效
 * @throws {Error} 如果数据库操作失败
 */
export async function getUserProfile(username: string): Promise<{ username: string; email: string; createdAt: string; role: string; lastLoginAt: string | null; uid: string; } | null> {
  if (!username) {
    throw new ValidationError('username 不能为空');
  }
  const userPath = `users/${username}`;
  try {
    const userData = await read(userPath);
    if (!userData) {
      // 统一错误信息，不直接告知用户不存在
      // throw new ValidationError('用户不存在'); // 更改为返回 null，因为这是一个查询接口
      return null;
    }
    // 返回不包含密码哈希的敏感信息
    const { password, ...safeUserData } = userData; // 解构去除密码
    return { ...safeUserData, uid: username } as { username: string; email: string; createdAt: string; role: string; lastLoginAt: string | null; uid: string; };
  } catch (error: unknown) {
    if (error instanceof ValidationError) {
      throw error;
    }
 ...
 */
export default async function userApiHandler(req: Request, res: Response): Promise<Response> {
  // 设置 CORS 头，处理预检请求
  setCORS(res, req);
