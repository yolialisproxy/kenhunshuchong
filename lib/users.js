 ...
 */
export async function deleteUser({ username }) { // 修正参数为对象解构
  if (!validateInput(username, 'username')) {
    logger.warn('[UsersLib] 删除用户：无效的 username', { username });
...
export async function loginUser(credentials) { // 保持对象参数，无需修改签名
  ...
  if (!validateInput(username, 'username')) {
    logger.warn(`[UsersLib] 登录失败：用户名无效 (username: ${username})`);
 ...
 * @throws {Error} 如果数据库操作失败
 */
export async function getUserProfile({ username }) { // 修正参数为对象解构
  if (!validateInput(username, 'username')) {
    logger.warn('[UsersLib] 获取用户资料：无效的 username', { username });
    throw new ValidationError('Invalid username format');
  }
 ...
 * @throws {Error} 如果数据库操作失败
 */
export async function updateUser({ username, ...updateFields }) { // 修正参数为对象解构，明确分离 username 和 updateFields
  if (!validateInput(username, 'username')) {
    logger.warn('[UsersLib] 更新用户：无效的 username (目标用户)', { username });
    throw new ValidationError('Invalid username format for target user');
  }
