...
export async function loginUser(credentials) {
  ...
  try {
    ...
    await write(userPath, { isActive: true }, { method: 'update' });
...
export async function logoutUser(credentials) {
  ...
  try {
    ...
    if (!userData) {
      logger.warn('[UsersLib] 登出失败：用户不存在', { username });
      throw new ValidationError('Username or password incorrect.'); // Generic error for security
    }

    await write(userPath, { isActive: false }, { method: 'update' });

    logger.info('[UsersLib] 登出成功', { username });

    return { username: username };

  } catch (error) {
    if (error instanceof ValidationError) throw error; // Re-throw validation errors
    logger.error('[UsersLib] 登出失败', error, { username });
    // Provide more specific error messages for common issues
