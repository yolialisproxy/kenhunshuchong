...
export default async function userApiHandler(req, res) { // 将 userApiHandler 默认导出
  ...
  if (req.method === 'OPTIONS') {
    return new Response(null, { status: 204, headers: { ...res.headers.raw(), ...responseHeaders } }); // 合并 CORS 头
  }
...
export {
  ...
  getUserProfile,
  updateUser,
  ValidationError // 导出 ValidationError for consistent error handling
};
