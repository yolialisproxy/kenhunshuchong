...
// 计算总赞数 (业务层调用utils，修复：从数据库读取 floor 作为 depth)
export async function computeCommentTreeTotalLikes(postId, commentId) {
  ...
  try {
    ...
    if (commentData.totalLikes !== newTotalLikes) {
      ...
      logger.info(`更新总赞数 ( postId: ${postId}, commentId: ${commentId}, totalLikes: ${newTotalLikes}, floor: ${depth} )`);
    } else {
      logger.info(`总赞数未变化 ( postId: ${postId}, commentId: ${commentId}, floor: ${depth} )`);
    }
...
// 状态回顾：2025-08-23优化，新增楼层（floor）功能，顶层评论递增计数，子评论为 null。
...
export async function addComment(commentData) {
  ...
  try {
    ...
    let fullData;
    await transaction(metadataPath, function(current) {
      // 获取当前楼层计数器，初始为 0
      const floorCounter = current?.counter || 0;
      const newFloor = parentId === '0' ? floorCounter + 1 : null;

      // 更新计数器（仅顶层评论）
      if (parentId === '0') {
        return { counter: floorCounter + 1 };
      }
      return current;
    }).then(async () => {
      // 获取最新楼层号
      const metadata = await read(metadataPath);
      const floor = parentId === '0' ? (metadata?.counter || 1) : null;
    ...
    logger.success(`评论添加成功 (postId: ${postId}, commentId: ${commentId}, floor: ${fullData.floor || '子评论'})`);
...
// 删除评论
export async function removeComment(postId, commentId, username, adminId) {
  adminId = adminId || 'yolialisproxy';
  if (!validateInput(postId, 'id')) throw new ValidationError('文章ID无效');
  if (!validateInput(commentId, 'id')) throw new ValidationError('评论ID无效');
  if (!username) throw new ValidationError('username不能为空');
  ...
  try {
    ...
    const isAdmin = username === adminId;
...
export async function updateComment(postId, commentId, content, username) {
  ...
  try {
    ...
    const isAuthor = commentData.name === username;
    const isAdmin = username === 'yolialisproxy';
