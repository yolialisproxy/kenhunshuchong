...
async function computeTotalLikes(postId, commentId, depth = 0) {
  ...
  try {
    ...
    await withTimeout(update(commentRef, { totalLikes: total }), 3000);
    return total;
  } catch (err) {
...
export async function submitComment(req, res) {
  setCORS(res, ['https://*.vercel.app', 'http://localhost:1313']);
...
export async function editComment(req, res) {
  ...
  try {
    ...
    await withTimeout(update(commentRef, { comment }), 3000);
