...
* @param {string} commentData.name - 用户名 (或游客名)
* @param {string} commentData.email - 用户邮箱 (或游客邮箱)
* @param {string} commentData.content - 评论内容
* @param {string} [commentData.parentId='0'] - 父评论ID，'0' 表示顶层评论
* @param {boolean} [commentData.isGuest=false] - 是否为游客评论
* @returns {Promise<object>} - 新评论数据
* @throws {ValidationError} 如果输入无效
* @throws {Error} 如果数据库操作失败
*/
export async function addComment({ postId, name, email, content, parentId = '0', isGuest = false }) {
  // --- 1. Input Validation ---
  if (!postId || !validateInput(postId, 'id')) throw new ValidationError('无效的文章ID');
  if (!name || !validateInput(name, 'name')) throw new ValidationError('用户名无效 (1-50字符)');
  ...
  // parentId 验证，允许 '0' 或有效ID
  if (parentId !== '0' && !validateInput(parentId, 'id')) throw new ValidationError('无效的父评论ID');

  const commentId = `${Date.now()}_${Math.random().toString(36).substring(2, 9)}`; // Unique ID
  const commentPath = `comments/${postId}/${commentId}`;
  const parentPath = parentId === '0' ? `comments/${postId}` : `comments/${postId}/${parentId}`;

  try {
    // --- 2. Check Parent Comment Existence (if not top-level) ---
    if (parentId !== '0') {
      const parentComment = await read(parentPath);
      if (!parentComment) {
        logger.warn(`[CommentsLib] addComment: 父评论不存在 (postId: ${postId}, parentId: ${parentId})`);
    ...
    // --- 4. Construct New Comment Object ---
    const newComment = {
      commentId,
      postId,
      name,
      ...
      totalLikes: 0, // Total likes for this comment and its children
      children: {}, // Placeholder for nested comments
      floor: floor // Only for top-level comments
    };

    // --- 5. Atomically Add Comment and Update Parent's Children ---
    await transaction(commentPath, (current) => {
      if (current) {
        logger.warn(`[CommentsLib] addComment: 评论ID冲突，尝试重新生成 (commentId: ${commentId})`);
        return undefined; // Abort transaction
      }
      return newComment;
    });

    // Atomically add child reference to parent
    if (parentId !== '0') {
      await transaction(`${parentPath}/children`, (currentChildren) => {
        const children = currentChildren || {};
        children[commentId] = true; // Store a boolean or a minimal reference
        return children;
      });
    }

    logger.info(`[CommentsLib] addComment: 评论添加成功 (commentId: ${commentId}, postId: ${postId})`);

    // --- 6. Initialize totalLikes for the new comment ---
    await write(`${commentPath}/totalLikes`, 0, { method: 'set' }); // Initialize totalLikes to 0

    // Return the newly created comment object
    return newComment;

  } catch (error) {
    logger.error(`[CommentsLib] addComment: 添加评论失败 (postId: ${postId}, name: ${name}):`, error);
    if (error instanceof ValidationError) throw error;
    // FIX: 修正语法错误 new new Error 为 new Error
