 ...
 */
export async function computeCommentTreeTotalLikes({ postId, commentId }) { // 修正参数为对象解构
  if (!validateInput(postId, 'id') || !validateInput(commentId, 'id')) {
    logger.warn(`[CommentsLib] computeCommentTreeTotalLikes: 无效的ID (postId: ${postId}, commentId: ${commentId})`);
 ...
 * @throws {Error} 如果更新失败
 */
export async function updateCommentAncestorsTotalLikes({ postId, initialCommentId }) { // 修正参数为对象解构
  if (!validateInput(postId, 'id') || !validateInput(initialCommentId, 'id')) {
    logger.warn(`[CommentsLib] updateCommentAncestorsTotalLikes: 无效的ID (postId: ${postId}, initialCommentId: ${initialCommentId})`);
    throw new ValidationError('无效的文章ID或评论ID');
  }
 ...
 * @throws {Error} 如果数据库操作失败
 */
export async function getComments({ postId }) { // 修正参数为对象解构
 ...
 * @throws {Error} 如果数据库操作失败
 */
export async function deleteComment({ postId, commentId, username, adminUsername = 'yolialisproxy' }) { // 修正参数为对象解构
  if (!validateInput(postId, 'id')) throw new ValidationError('文章ID无效');
  if (!validateInput(commentId, 'id')) throw new ValidationError('评论ID无效');
  if (!username || !validateInput(username, 'username')) throw new ValidationError('请求用户名无效或为空');
 ...
 * @throws {Error} 如果数据库操作失败
 */
export async function updateComment({ postId, commentId, content, username, adminUsername = 'yolialisproxy' }) { // 修正参数为对象解构
  if (!validateInput(postId, 'id')) throw new ValidationError('文章ID无效');
  if (!validateInput(commentId, 'id')) throw new ValidationError('评论ID无效');
  if (!validateInput(content, 'comment')) throw new ValidationError('评论内容无效 (需5-500字符)');
  if (!username || !validateInput(username, 'username')) throw new ValidationError('请求用户名无效或为空');
