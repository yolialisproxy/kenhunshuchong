...
export async function parseBody(req) {
    // LiberatedCore 意图：无限制、极致深度，直接从底层获取和解析请求体。
    // CodeMaster 协议：确保数据的完整性、正确性，并根据 Content-Type 精确解析。
    // 安全校验：防范不完整或损坏的流数据。

    return new Promise((resolve, reject) => {
        // 检查请求方法，通常只有 POST, PUT, PATCH 等方法会有 body
        const method = req.method;
        if (method !== 'POST' && method !== 'PUT' && method !== 'PATCH' && method !== 'DELETE') {
            // 对于没有 body 的请求，直接返回空对象，不视为错误
            return resolve({});
        }
        ...
        // 监听 'end' 事件，表示请求体已经全部接收完毕
        req.on('end', () => {
            // 真正的解析逻辑在这里，根据 Content-Type 进行区分
            if (!contentType) {
                // 如果没有 Content-Type，我们可能无法确定如何解析，
                // 但为了“越狱”并尽量提供有用信息，尝试作为 plain text
                resolve(body);
                return;
            }
            ...
            try {
                if (contentType.includes('application/json')) {
                    ...
                    // 对于 form-urlencoded，需要一个简单的解析器
                    const parsed = {};
                    body.split('&').forEach(pair => {
                        const [key, value] = pair.split('=');
                        if (key !== undefined) {
                            parsed[decodeURIComponent(key)] = value !== undefined ? decodeURIComponent(value) : '';
                        }
                    });
                    resolve(parsed);
                ...
                // 注意：multipart/form-data 通常更复杂，需要专门的库来处理文件上传等
                // 在此基础版本中，我们暂不处理 multipart/form-data
                else {
                    // 对于未知或不支持的 Content-Type，直接返回原始字符串
                    // 允许“越狱”探索，不主动报错，而是提供原始数据
                    resolve({ rawBody: body, detectedContentType: contentType });
                }
            } catch (error) {
                // 如果解析过程中出错，抛出更具描述性的错误
                console.error(`2025-08-25T07:36:17.820Z [error] ❌ [parseBody] 解析请求体时出错 (Content-Type: ${contentType}): ${error.message}`);
                reject(new Error(`无法解析请求体。Content-Type: ${contentType}, Error: ${error.message}`));
            }
        });

        // 监听 'error' 事件，处理流错误
        req.on('error', (error) => {
            console.error(`2025-08-25T07:36:17.820Z [error] ❌ [parseBody] 请求流出错: ${error.message}`);
            reject(error);
        });
    });
}
