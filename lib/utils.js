...
export async function parseBody(req) {
  // Check if req and req.headers are valid before proceeding
  if (!req || !req.headers) {
    logger.error('parseBody: req 或 req.headers 无效，无法继续解析。');
    throw new Error('Invalid request context for parsing body');
  }

  // If req.body is already a parsed object (not a ReadableStream), return it directly.
  ...
  try {
    ...
    let parsedBody = {}; // Initialize parsedBody as an empty object

    if (contentType && contentType.includes('application/json')) {
      parsedBody = await req.json(); // Read and parse JSON
    } else if (contentType && contentType.includes('application/x-www-form-urlencoded')) {
      const text = await req.text(); // Read raw text
      const params = new URLSearchParams(text); // Parse form data
      parsedBody = Object.fromEntries(params.entries()); // Convert to object
    } else {
      // Handle cases where Content-Type is missing or unsupported
      logger.warn('parseBody: 未处理的 Content-Type 或 Content-Type 缺失', { contentType: contentType || '缺失' });
      // Return empty object for unsupported types, as per original logic
      // This is a 'graceful degradation' if no data is expected or parseable.
    }

    // --- Final Check: Ensure parsed body is a non-empty object if data was expected ---
    // If the operation is expected to have a body (e.g., POST, PUT), and the parsed body is empty,
    // it might indicate an issue, but returning {} is often safer than crashing.
    // For now, we'll just log if the body is empty after parsing.
    if (Object.keys(parsedBody).length === 0 && (req.method === 'POST' || req.method === 'PUT')) {
      logger.warn('parseBody: 解析后的请求体为空，但方法是 POST 或 PUT', { url: req.url, method: req.method, contentType });
    }

    return parsedBody; // Return the parsed body, or an empty object if parsing failed/unsupported

  } catch (e) {
    // Catch any errors during parsing (e.g., invalid JSON, stream issues)
    logger.error('parseBody: 请求体解析过程中发生错误', e, { url: req.url, method: req.method, contentType: req.headers.get('content-type') });
