...
export const CONFIG = {
  ...
  // 新增/修改：简化评论验证配置
  COMMENT_VALIDATION: {
      name: { min: 1, max: 50 }, // 姓名只需要非空，长度在合理范围内
      username: { min: 1, max: 30 }, // 用户名只需要非空，长度在合理范围内
      email: { regex: /^[^\s@]+@[^\s@]+\.[^\s@]+$/ },
...
export function getCache(key) {
  ...
  if (entry && Date.now() - entry.timestamp < CONFIG.CACHE_TTL * 1000) {
    logger.info('从缓存命中', { key }); return entry.value;
  }
  cache.delete(key); logger.info('缓存未命中或过期', { key }); return null;
}
...
export async function initFirebase() {
  if (db) {
    logger.info('Firebase已初始化，直接返回db实例'); return db;
  }
  try {
    validateEnv();
    const firebaseConfig = {
      ...
      projectId: process.env.FIREBASE_PROJECT_ID, storageBucket: process.env.FIREBASE_STORAGE_BUCKET, messagingSenderId: process.env.FIREBASE_MESSAGING_SENDER_ID,
      appId: process.env.FIREBASE_APP_ID
    };
    app = initializeApp(firebaseConfig); db = getDatabase(app);
    logger.success('Firebase初始化成功'); return db;
  } catch (err) {
    logger.error('Firebase初始化失败:', err); throw err;
  }
}
...
export async function read(path, options) {
  ...
  const cacheKey = 'read:' + path; const cached = getCache(cacheKey);
  if (cached !== null) { logger.info('从缓存读取: ' + path); return cached; }
  try {
    const dbInstance = await initFirebase();
    const snapshot = await withTimeout(get(ref(dbInstance, path)), timeout);
    const value = snapshot.val() || null; setCache(cacheKey, value);
    logger.info('数据库读取成功 (路径: ' + path + ')'); return value;
  } catch (err) {
    logger.error('数据库读取失败 (路径: ' + path + '):', err); throw err;
  }
}
...
export async function write(path, data, options) {
  ...
  try {
    ...
    if (method === 'push' && newRef) return { key: newRef.key }; return;
  } catch (err) {
    logger.error('数据库写入失败 (路径: ' + path + '):', err); throw err;
  }
}
...
export async function transaction(path, updateFn, options) {
  ...
  try {
    ...
    invalidateCache('read:' + path); return result;
  } catch (err) {
    logger.error('事务操作失败 (路径: ' + path + '):', err); throw err;
  }
}
...
export async function del(path, options) {
  ...
  try {
    ...
    logger.error('数据删除失败 (路径: ' + path + '):', err); throw err;
  }
}

// --- 输入处理与验证 ---
export function sanitizeInput(input) {
  if (typeof input !== 'string') { logger.warn('sanitizeInput: 输入不是字符串', { input }); return ''; }
  try {
    let sanitized = input.replace(/<script\b[^<]*(?:(?!<\/script>)<[^<]*)*<\/script>/gi, ''); // 移除 script 标签
    sanitized = sanitized.replace(/<[^>]+>/g, ''); // 移除所有其他 HTML 标签
    // 移除除字母、数字、空格、. _ @ - 之外的所有字符（支持Unicode）
    ...
    logger.error('sanitizeInput 失败', error, { input }); return '';
  }
}
...
export function validateInput(value, type) {
  ...
  // name 和 username 类型不进行 sanitizeInput，直接使用原始值验证
  ...
  // 仅对 comment 类型进行 sanitizeInput
  if (type === 'comment') {
      processedValue = sanitizeInput(value);
  }

  switch (type) {
    case 'name': // 姓名只需要非空，长度在合理范围内
      return processedValue.length >= config.min && processedValue.length <= config.max;
    case 'username': // 用户名只需要非空，长度在合理范围内
      return processedValue.length >= config.min && processedValue.length <= config.max;
    case 'email':
...
export async function parseBody(req) {
  ...
  try {
    ...
    if (contentType && contentType.includes('application/x-www-form-urlencoded')) {
      const text = await req.text(); const params = new URLSearchParams(text);
      return Object.fromEntries(params.entries());
    }
    logger.warn('parseBody: 未处理的 Content-Type: ' + (contentType || '缺失')); return {};
  } catch (e) {
    logger.error('parseBody: 请求体解析失败:', e); return {};
  }
}
...
export async function computeTotalLikes(postId, commentId, startDepth = 0) {
  ...
  try {
    ...
    if (!commentData) { logger.warn('computeTotalLikes: 评论不存在，无法计算点赞数', { postId, commentId }); return 0; }
    ...
    logger.error('computeTotalLikes: 计算失败', { postId, commentId, error: err.message, stack: err.stack }); throw err;
  }
}
