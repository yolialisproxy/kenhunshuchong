...
export function setCORS(req) {
  ...
  // 考虑更宽松的本地开发环境，允许所有 localhost
  const allowedOrigins = (env.NODE_ENV !== 'production'
    ? ['http://localhost:3000', 'http://localhost:8080', 'http://127.0.0.1:3000', 'http://127.0.0.1:8080']
    : (env.CORS_ALLOWED_ORIGINS || 'https://kenhunshuchong.vercel.app').split(',')
  );
  ...
  if (origin) {
    // Process wildcard domains into regular expressions for matching
    const wildcardDomains = productionAllowedOrigins.filter(o => o.includes('*'));
    originAllowed = allowedOrigins.some(allowedOrigin => {
        if (allowedOrigin.includes('*')) {
            // Process wildcard into a RegExp: replace '.' with '\.' and '*' with '.*'
            // Corrected line: Removed the extra ```javascript suffix
            const regex = new RegExp(`^${allowedOrigin.replace(/\./g, '\\.').replace(/\*/g, '.*')}$`, 'i'); // Added 'i' flag for case-insensitive matching
  ...
  // 确定 Access-Control-Allow-Origin 的值
  // 如果 originAllowed 为 true，则允许原始 origin
  // 如果是非生产环境，则允许原始 origin 或 '*'
  // 如果是生产环境且 originAllowed 为 false，则设置为 'null'
  const allowOrigin = originAllowed ? origin : (env.NODE_ENV !== 'production' ? origin || '*' : 'null');

  const headers = {
    'Access-Control-Allow-Origin': allowOrigin, // 允许的源
    'Access-Control-Allow-Methods': 'GET, POST, PUT, DELETE, OPTIONS', // 支持的方法
