...
export async function parseBody(req) {
  return new Promise((resolve, reject) => {
    if (req.body) { // Vercel might pre-parse JSON bodies
      ...
      try {
        ...
        // Fall through to stream parsing if it's not JSON
      }
    }
    ...
    req.on('end', () => {
      if (!body) {
        // If no body, check query parameters for GET requests
        ...
        return resolve({}); // No body, no query params
      }
      try {
        resolve(JSON.parse(body));
      } catch (e) {
        // If not JSON, try URL-encoded (common for forms)
        try {
          ...
          reject(new Error('Failed to parse request body as JSON or URL-encoded.'));
        }
      }
    });
    req.on('error', err => {
