...
const logger = {
  ...
  error: (message, error, meta = {}) => console.error(`❌ [ERROR] Utils: ${message}`, meta, error ? error.stack : '') // 修正 error 记录
};
...
export async function initFirebase() {
  ...
  try {
    ...
    logger.error('Firebase初始化失败:', err); // 直接传递错误对象
    throw err;
  }
}
...
// 移除了 typedef {Object.<string, any>} Data 通用数据类型
// 移除了 typedef {Error & { code?: string }} FirebaseError 扩展的Firebase错误类型
...
export async function read(
  ...
  try {
    ...
    if (err instanceof Error) {
      logger.error(`数据库读取失败 (路径: ${path}):`, err); // 直接传递错误对象
      throw err;
    }
...
export async function write(
  ...
  try {
    ...
    if (err instanceof Error) {
      logger.error(`数据库写入失败 (路径: ${path}):`, err); // 直接传递错误对象
      throw err;
    }
...
export async function transaction(
  ...
  try {
    ...
    if (err instanceof Error) {
      logger.error(`事务操作失败 (路径: ${path}):`, err); // 直接传递错误对象
      throw err;
    }
...
export async function del(
  ...
  try {
    ...
    if (err instanceof Error) {
      logger.error(`数据删除失败 (路径: ${path}):`, err); // 直接传递错误对象
      throw err;
    }
...
export async function parseBody(req) {
  ...
  try {
    ...
    logger.warn('Request body parsing failed:', e); // 直接传递错误对象
    return {};
  }
}
...
export function setCORS(res, req) {
  ...
  if (origin) {
    originAllowed = allowedOrigins.some(ao => {
      ...
      if (ao.startsWith('https://*.') && origin.startsWith('https://') && origin.endsWith(ao.substring(9))) {
        // 确保匹配的是一个完整的子域名，而不是 'https://.example.com'
        const subdomainPart = origin.substring('https://'.length, origin.length - ao.substring(9).length);
        return subdomainPart.indexOf('.') === -1; // 确保只有一级子域名
      }
      return false;
    });
  }
