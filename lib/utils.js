...
export function setCORS(req) {
  // --- Get Origin Header Safely ---
  // Access headers directly using property access, ensuring the key is lowercase.
  // Vercel environment typically lowercases headers internally.
  const origin = req.headers && typeof req.headers === 'object'
                 ? req.headers['origin']?.toLowerCase()
                 : null;

  // Log the origin value for debugging purposes
  if (!origin) {
  ...
  if (origin) {
    originAllowed = allowedOrigins.some(allowedOrigin => {
        if (allowedOrigin.includes('*')) {
            // Convert wildcard to RegExp for matching
            const regex = new RegExp(`^${allowedOrigin.replace(/\./g, '\\.').replace(/\*/g, '.*')}$`, 'i');
            return regex.test(origin);
        } else {
            return allowedOrigin === origin; // Exact match
        }
    });
  }
