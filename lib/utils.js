...
export function setCORS(req) {
  let origin = null;
  // Safely access origin: first try Headers.get, then fallback to object property access
  if (req && req.headers) {
    if (typeof req.headers.get === 'function') {
      origin = req.headers.get('origin')?.toLowerCase();
    }
    // Fallback if .get() is not available or returns undefined
    if (origin === null || origin === undefined) {
      // Directly access via property, assuming headers are lowercased
      origin = req.headers['origin']?.toLowerCase();
    }
  }

  if (!origin) {
    // Log if origin could not be determined
    logger.warn('[Utils] setCORS: 无法从请求头中获取 origin', { headers: req.headers });
  }

  // Define allowed origins (adjust based on your deployment environment)
