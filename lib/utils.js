...
import pkg from 'sanitize-html'; // 修复：使用默认导入
const { sanitizeHtml } = pkg; // 解构默认导出的 sanitizeHtml
...
// 设置CORS跨域头（兼容 Vercel 的 res 结构，修复 req.headers.get）
export function setCORS(res, req) {
  ...
  // 使用 req.headers.origin 替代 req.headers.get('Origin')
  const origin = req.headers && req.headers.origin ? req.headers.origin.toLowerCase() : '';

  // 检查 res 是否有 headers 属性
  if (!res || !res.headers || typeof res.headers.set !== 'function') {
    logger.warn('setCORS: Invalid res object, skipping headers setup', { res: !!res, headers: !!res?.headers });
    return;
  }
  ...
  // 设置 CORS 头
  res.headers.set('Access-Control-Allow-Origin', originAllowed || process.env.NODE_ENV !== 'production' ? origin || '*' : '*');
