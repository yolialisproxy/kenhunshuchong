...
export function validateInput(value, type) {
  ...
  switch (type) {
    ...
    case 'username':
      ...
      return processedValue.length >= (config.min || 0) && processedValue.length <= (config.max || Infinity);
    case 'email':
      // 使用正则表达式验证邮箱格式
    ...
    case 'password':
      ...
      if (processedValue.length < (config.min || 0) || processedValue.length > (config.max || Infinity)) passwordValid = false;
    ...
    case 'id':
      // 验证 ID 的格式和长度
      return config.regex.test(processedValue) && processedValue.length >= (config.min || 0) && processedValue.length <= (config.max || Infinity);
    default:
...
export async function parseBody(req) {
  // 如果 req.body 已经是解析过的对象且不是 ReadableStream，直接返回
  // 'pipeTo' in req.body 检查是否是 ReadableStream
  ...
  try {
    ...
    if (contentType && contentType.includes('application/json')) {
      ...
      // 如果 Content-Type 未知或不支持，记录警告并返回空对象
...
* 设置 CORS 响应头。
* @param {Request} req - Vercel Serverless Function 的 Request 对象。
* @returns {object} - 包含 CORS 响应头的对象。
*/
export function setCORS(req) {
  // --- 尝试使用 req.headers.get('origin') ---
  // This is the standard way for Web Request objects. If it fails, we fall back.
  let origin = req.headers.get ? req.headers.get('origin')?.toLowerCase() : null;

  // --- Fallback for environments where req.headers might not have .get() ---
  // If req.headers.get failed or is not available, try accessing it as a plain object.
  if (origin === null || origin === undefined) {
    // Check if req.headers itself exists and is an object
    if (req.headers && typeof req.headers === 'object') {
      // Access headers directly as a property. Header names are typically lowercased by Node.js/Vercel.
      origin = req.headers['origin']?.toLowerCase();
    }
    if (!origin) {
      logger.warn('[Utils] setCORS: 无法从请求头中获取 origin (尝试了 .get() 和直接访问)', { headers: req.headers });
    }
  }

  // --- Rest of the CORS logic remains the same ---
  const allowedOrigins = (process.env.NODE_ENV !== 'production'
    ? ['http://localhost:3000', 'http://localhost:8080', 'http://127.0.0.1:3000', 'http://127.0.0.1:8080', 'https://kenhunshuchong.web.app']
    : (process.env.CORS_ALLOWED_ORIGINS || 'https://kenhunshuchong.vercel.app').split(',')
  );

  let originAllowed = false;

  if (origin) {
    originAllowed = allowedOrigins.some(allowedOrigin => {
